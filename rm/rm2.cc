...
{
	cout<<"55555.22.1"<<endl;

	rids.push_back(rid);

	cout<<"55555.22.22"<<endl;
}
...
void display_output(void *projection,int pro_offset)
{
	int x,offset=0;

	fout<<"actual size of tuple:"<<pro_offset<<endl;

	memcpy(&x,(char*)projection+offset,sizeof(int));
	offset+=sizeof(int);
	...
	memcpy(string,(char*)projection+offset,x);
	offset+=x;
...
RC RM::scan(const string tableName,const string conditionAttribute,const CompOp compOp,const void *value,const vector<string> &attributeNames,RM_ScanIterator &rm_ScanIterator)
{
	...
	int i,nop,j,k,offset,x,pro_offset,flag,y;
	...
	char *projection_cstring = (char*)malloc(100);
	...
	for(k=0;k<(int)attrs.size();k++)
	{
		if(project(attrs.at(k).name,attributeNames)==0)
		{
			...
			fout<<"Name :"<<attrs[k].name<<endl;
			fout<<"Type :"<<attrs[k].type<<endl;
			fout<<"Length :"<<attrs[k].length<<endl;
...
/*
				if(conditionAttribute.length()==0)
				{
					fout<<"Conditional Attribute Null"<<endl;
					flag = 1;
				}
*/
				if(attrs.at(k).name.compare(conditionAttribute)==0)
				{
			...
			for(k=0;k<(int)attrs.size();k++)
			{
				...
				//cout<<"Attrs name :"<<attrs[k].name<<endl;
				//cout<<"Attrs type :"<<attrs[k].type<<endl;
				//cout<<"Attrs length :"<<attrs[k].length<<endl;

				if(project(attrs.at(k).name,attributeNames) == 0)
				{		
					//cout<<"TEST:"<<attrs.at(k).name<<endl;

					fout<<"Attrs name :"<<attrs[k].name<<endl;
					fout<<"Attrs type :"<<attrs[k].type<<endl;
					fout<<"Attrs length :"<<attrs[k].length<<endl;
				...
				{
					cout<<"7777777.4444"<<endl;

					if(attrs.at(k).type == TypeVarChar)
					{
						cout<<"7777777.55555"<<endl;
						...
						cout<<"7777777.666666"<<endl;
					}
					else
					{
						cout<<"7777777.7777777"<<endl;
...
//			memcpy(projection_cstring,(char*)projection,pro_offset);
			...
			display_output(projection,pro_offset);
