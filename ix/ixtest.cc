...
{    
    ...
    for(unsigned i = 0; i <= numOfTuples; i=i+20) 
    {
    ...
    for(unsigned i = 10; i <= numOfTuples; i=i+20) 
    {
        unsigned key = i+1;//just in case somebody starts pageNum and recordId from 1
        RID rid;
        rid.pageNum = key;
...
{    
	...
	//cout<<"Inserted in test5:"<<i+1<<endl;    
    }
    ...
    rc= ixScan->OpenScan(ixHandle, LE_OP, &value);
    if(rc == success)
    {
        cout << "Scan Opened Successfully!" << endl;
    }
    ...
    if(rc == success)
    {
        cout << "Index Destroyed Successfully! in tesst case 5" << endl;
    }
    else 
    {
        cout << "Failed Destroying Index...in test case 5" << endl;
    }
    return;
}
...
void test_entry()
{
	...
	int x = pf->OpenFile("tbl_employee.Height",fileHandle);
	...
	for(int i=1;i<471;i++)
	{
		x = fileHandle.ReadPage(i,data);

		memcpy(&x,(char*)data,sizeof(int));

		if(x == 0)
		{
			cout<<"Displaying contents of non-leaf node<"<<i<<">:"<<endl;
			memcpy(&N,(char*)data+4,sizeof(N));
			//display_node(N);
		}
		else
		{
			cout<<"Displaying contents of leaf node<"<<i<<">:"<<endl;
