...
/*
		if((*handle).IsFree()==0)
			cout<<"this->Handle is free!"<<endl;
		else
			cout<<"this->Handle is not free!"<<endl;
*/
        };
...
class HashJoin : public Iterator {
	...
	int iterator,count;
        ...
        HashJoin(Iterator *leftIn,                                // Iterator of input R
                 ...
                 const Condition &condition,                      // Join condition
                 unsigned numPages                          // Number of pages can be used to do join (decided by the optimizer)
        );
